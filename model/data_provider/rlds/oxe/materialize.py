...
from model.data_provider.rlds.oxe.configs import OXE_DATASET_CONFIGS, ActionEncoding
from model.data_provider.rlds.oxe.transforms import OXE_STANDARDIZATION_TRANSFORMS
from model.data_provider.rlds.utils.data_utils import NormalizationType
...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.EEF_POS, ActionEncoding.EEF_R6, ActionEncoding.DEXTEROUS, ActionEncoding.EEF_POSE, ActionEncoding.BIMANUAL_EEF_POSE, ActionEncoding.JOINT_POS_BIMANUAL]:
        raise ValueError(f"Cannot load `{dataset_name}`; only EEF_POS & EEF_R6 & DEXTEROUS & EEF_POSE & BIMANUAL_EEF_POSE actions supported!")
    ...
    elif dataset_kwargs["action_encoding"] is ActionEncoding.DEXTEROUS:
        dataset_kwargs["absolute_action_mask"] = [False] * 22
        dataset_kwargs["action_normalization_mask"] = [True] * 22
    elif dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POSE:  # 7 delta pose + 1 gripper
        dataset_kwargs["absolute_action_mask"] = [False] * 7 + [True]
        dataset_kwargs["action_normalization_mask"] = [True] * 7 + [False]
    elif dataset_kwargs["action_encoding"] is ActionEncoding.BIMANUAL_EEF_POSE:  # 2 * (7 delta pose + 1 gripper)
        dataset_kwargs["absolute_action_mask"] = [False] * 7 + [True] + [False] * 7 + [True]
        dataset_kwargs["action_normalization_mask"] = [True] * 7 + [False] + [True] * 7 + [False]
    ...
    dataset = dataset_name[6:] if dataset_name.startswith("aloha_") else dataset_name
    return {"name": dataset_name, "data_dir": str(Path(data_root_dir) / dataset / "1.0.0"), **dataset_kwargs}
