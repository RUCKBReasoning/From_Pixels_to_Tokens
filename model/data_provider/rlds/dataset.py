...
from model.data_provider.rlds import obs_transforms, traj_transforms
from model.data_provider.rlds.utils import goal_relabeling, task_augmentation
from model.data_provider.rlds.utils.data_utils import (
    NormalizationType,
    allocate_threads,
...
from model.models.constants import ACTION_PROPRIO_NORMALIZATION_TYPE
import random
...
def sample_exact_by_instruction(dataset, fraction, seed=None):
    all_trajs = list(dataset.as_numpy_iterator())
    # group by instruction
    groups = {}
    for traj in all_trajs:
        instr = traj["task"]["language_instruction"][0].decode("utf-8")
        groups.setdefault(instr, []).append(traj)

    # sample on each group
    rng = random.Random(seed)
    sampled = []
    for instr, trajs in groups.items():
        k = max(1, int(len(trajs) * fraction))
        sampled.extend(rng.sample(trajs, k))

    # reconstruct dataset
    return _wrap(tf.data.Dataset.from_generator, False)(
        lambda: iter(sampled),
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        # goal image calculate
        goal_indices = tf.minimum(new_obs["timestep"] + goal_image_step, traj_len - 1)
        goal_images = {}
        for new in image_obs_keys.keys():
            img_key = f"image_{new}"
            if goal_image_step > 0:
                goal_images[img_key] = tf.gather(new_obs[img_key], goal_indices)
            else:
                goal_images[img_key] = new_obs[img_key]
        ...
        traj = {
            ...
            "latent_idx": tf.cast(traj["latent_idx"], tf.float32),
            "latent_z": tf.cast(traj["latent_z"], tf.float32),
        }
...
def apply_frame_transforms(
    ...
    *,
    ...
    def apply_obs_transform(fn: Callable[[Dict], Dict], frame: Dict, goal_image=True) -> Dict:
        frame["task"] = fn(frame["task"])
        frame["observation"] = dl.vmap(fn)(frame["observation"])
        if "goal_image" in frame:
            frame["goal_image"] = fn(frame["goal_image"])   # we assume goal images are 256x256
        return frame
    ...
    if train:
        ...
        def aug(frame: dict):
            ...
            return apply_obs_transform(aug_fn, frame, goal_image=False) # do not augment goal images. this function will automatically avoid augment depth images
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        print(f"[DEBUG] Loading dataset: {dataset_kwargs['name']}", flush=True)
        ...
        print(f"[DEBUG] Dataset size: {dataset_statistics['num_transitions']} transitions", flush=True)
