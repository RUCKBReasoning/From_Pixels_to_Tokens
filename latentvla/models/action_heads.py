...
try:
    ...
    def fast_layer_norm(x, weight, bias, eps=1e-6):
...
class LatentL2Projector(nn.Module):
    """
    Align action token embeddings with latent_action_z using L2 loss.
    """
    def __init__(self, llm_dim=2048, latent_dim=128):
        super().__init__()

        self.projector = nn.Sequential(
            nn.Linear(llm_dim, 2 * latent_dim),
            nn.GELU(),
    ...
    def forward(self, llm_action_tokens, latent_action_z):
        """
        llm_action_tokens: [B, 32, llm_dim]
        latent_action_z:   [B, 32, latent_dim]
        """
        proj = self.projector(llm_action_tokens)  # [B, 32, latent_dim]
        proj = F.normalize(proj, dim=-1)
        latent_action_z = F.normalize(latent_action_z, dim=-1)
        loss = F.mse_loss(proj, latent_action_z)
        return loss
...
class FastLayerNorm(nn.Module):
    def __init__(self, dim, eps=1e-6):
        ...
        self.eps = eps
    def forward(self, x):
        return fast_layer_norm(x, self.weight, self.bias, self.eps)
...
class MLPResNet(nn.Module):
    ...
    def __init__(
        ...
        # self.layer_norm1 = FastLayerNorm(input_dim)
        # self.layer_norm2 = FastLayerNorm(hidden_dim)
