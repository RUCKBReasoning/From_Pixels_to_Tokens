...
class Metrics:
    def __init__(
        ...
        hparams: Dict[str, Any],
        stage: str,
        wandb_project: str = "anonymous-project",
        ...
        for tracker_type in active_trackers:
            if tracker_type == "jsonl":
                ...
                tracker = WeightsBiasesTracker(
                    run_id, run_dir, hparams, project=wandb_project, entity=wandb_entity, group=self.stage, wandb=wandb
                )
            else:
...
class VLAMetrics:
    def __init__(
        ...
        for tracker_type in active_trackers:
            if tracker_type == "jsonl":
                ...
                tracker = WeightsBiasesTracker(
                    run_id, run_dir, hparams, project=wandb_project, entity=wandb_entity, group="vla-train", wandb=wandb
                )
            else:
                raise ValueError(f"Tracker with type `{tracker_type} is not supported!")
        ...
        self.state = {
            ...
            "latent_loss": deque(maxlen=window_size),
            "action_token_loss": deque(maxlen=window_size),
    ...
    def push(self) -> str:
        ...
        if len(self.state["action_loss"]) > 0:
            action_loss = torch.stack(list(self.state["action_loss"])).mean().item()
        else:
            action_loss = 0.0
        if len(self.state["latent_loss"]) > 0:
            latent_loss = torch.stack(list(self.state["latent_loss"])).mean().item()
        else:
            latent_loss = 0.0
        if len(self.state["action_token_loss"]) > 0:
            action_token_loss = torch.stack(list(self.state["action_token_loss"])).mean().item()
        else:
            action_token_loss = 0.0
        if len(self.state["val_loss"]) > 0:
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
                f"{prefix}/Latent Loss": latent_loss,
                f"{prefix}/Action Loss": action_loss,
                f"{prefix}/Action Token Loss": action_token_loss,
