...
def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    ...
    if "latent_idx" in traj:
        traj["latent_idx"] = tf.gather(traj["latent_idx"], floored_action_chunk_indices)
    if "latent_z" in traj:
        traj["latent_z"] = tf.gather(traj["latent_z"], floored_action_chunk_indices)
...
def add_pad_mask_dict(traj: Dict) -> Dict:
    ...
    for key in ["observation", "task", "goal_image"]:
        pad_mask_dict = {}
