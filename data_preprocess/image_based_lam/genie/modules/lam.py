...
class ControllableDINOLatentActionModel(nn.Module):
    ...
    def __init__(
        ...
        self.dino_transform = transforms.Normalize(mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD)
        # self.dino_encoder = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14_reg')
        checkpoint_path="path_to/dinov2_vitb14_reg4_pretrain.pth"
    ...
    def decode(self, z_q: Tensor, z_q_uncontrol: Tensor, start_frame: Tensor) -> Tensor:
        """
        Decodes a latent action into the predicted DINO features of the next frame.
        
        Args:
            z_q (Tensor): The quantized (or soft-averaged) controllable latent action codes. Shape: [B, 1, num_codes, latent_dim]
            z_q_uncontrol (Tensor): The quantized uncontrollable latent action codes. Shape: [B, 1, num_codes, latent_dim]
            start_frame (Tensor): The preprocessed starting image frame. Shape: [B, C, H, W]
            
        Returns:
            Tensor: The reconstructed DINO features of the target frame.
        """
        # Ensure start_frame is on the correct device
        start_frame = start_frame.to(self.device)

        # 1. Get DINO features for the start frame
        dino_features = self.dino_encoder.forward_features(start_frame)['x_norm_patchtokens']
        dino_features = dino_features.unsqueeze(1) # Add a time dimension -> [B, 1, L, D]

        # 2. Project features and action codes to the model dimension
        video_patches = self.patch_up(dino_features)
        action_patches_control = self.action_up(z_q)
        action_patches_uncontrol = self.action_up_uncontrol(z_q_uncontrol)
        
        # 3. Concatenate to form the input for the spatial decoder
        video_action_patches = torch.cat([action_patches_control, 
                                          action_patches_uncontrol, 
                                          video_patches],
                                          dim=2)

        # 4. Decode to get the reconstructed features of the next frame
        video_recon_features = self.decoder(video_action_patches)
        
        # 5. We only care about the image patch features, not the action features
        video_recon_features = video_recon_features[:, :, -video_patches.shape[2]:] 
        
        return video_recon_features.squeeze(1) # Remove time dimension
    @property
    def device(self):
        return next(self.parameters()).device